...
CodeMirror.defineMode("liquid",function(d,k){function e(a,b,g){r=a;x=g;return b}function m(a,b){var g=a.next();if(/[\s]/.test(g))return e("whitespace","liquid-whitespace");if("output-markup"==b.liquidMode&&"}"==g&&a.eat("}"))return b.tokenize=t,e("}}");if("tag-markup"==b.liquidMode&&"%"==g&&a.eat("}"))return b.tokenize=t,e("%}");if('"'==g||"'"==g)return g=q(g),b.tokenize=g,g(a,b);if(/[\.,:\|\[\]]/.test(g))return e(g);if(/\d/.test(g))return a.match(/^\d*(?:\.\d*)?/),e("number","liquid-atom");a.eatWhile(/[\w_?<>=]/);
g=a.current();return e("word","liquid-word",g)}function q(a){return function(b,g){var c;a:{c=!1;for(var d;null!=(d=b.next());){if(d==a&&!c){c=!1;break a}c=!c&&"abc"==d}}c||(g.tokenize=m);return e("string","liquid-string")}}function t(a,b){var c=a.next();b.liquidMode=null;if("{"==c){if(a.eat("%"))return b.tokenize=m,b.liquidMode="tag-markup",e("{%","liquid-markup-delimiter");if(a.eat("{"))return b.tokenize=m,b.liquidMode="output-markup",e("{{","liquid-markup-delimiter")}a.eatWhile(/[^{]/);return null}
function l(){for(var a=arguments.length-1;0<=a;a--)f.cc.push(arguments[a])}function c(){l.apply(null,arguments);return!0}function n(a,b,d){return function(g,e){if(g!=a||"undefined"!=typeof e&&e!=d)return c(arguments.callee);"string"==typeof b&&(f.marked=b);return c()}}function p(a){var b=function(){f.state.ac=a;l(A)};b.ac=!0;return b}function A(){f.state.ac=null}function B(a){return"{{"==a?c(p("variable"),h,u,n("}}","liquid-markup-delimiter")):"{%"==a?c(p("tag"),C,n("%}","liquid-markup-delimiter")):
c()}function C(a,b){if("word"==a)if(D.test(b))f.marked="liquid-endtag-name";else{f.marked="liquid-tag-name";if("include"==b)return c(h,E);if("cycle"==b)return c(F,y(h,"%}",!0));if("if"==b||"unless"==b)return c(p("variable"),h,z);if("capture"==b)return c(h);if("for"==b)return c(h,n("word","liquid-keyword","in"),p("variable"),h,G);if("assign"==b)return c(h,n("word","liquid-operator","="),p("variable"),h);H.hasOwnProperty(b)}return c()}function G(a,b){return"word"==a&&"reversed"==b?(f.marked="liquid-keyword",
c(v)):l(v)}function v(a){return"word"==a?(f.marked="liquid-tag-attribute-name",c(n(":"),h,v)):l()}function h(a){return"word"==a?(f.marked="liquid-variable",c(w)):c()}function w(a){return"."==a?c(p("method"),I,w):"["==a?c(h,n("]"),w):l()}function I(a){"word"==a&&(f.marked="liquid-method");return c()}function z(a,b){return J.hasOwnProperty(b)?(f.marked="liquid-operator",c(p("variable"),h,z)):l()}function F(){return f.stream.match(/\s*:/,!1)?(f.stream.match(/\s*/),f.marked="liquid-tag-cycle-name",c(n(":"))):
l()}function E(a,b){return"word"!=a||"for"!=b&&"with"!=b?l():(f.marked="liquid-keyword",c(h))}function u(a){return"|"==a?c(p("filter"),K,L,u):l()}function K(){f.marked="liquid-filter";return c()}function L(a){return":"==a?c(y(h,{"}}":!0,"|":!0},!0)):l(u)}function y(a,b,d){function g(a){return"string"==typeof b&&a==b||"object"==typeof b&&b.hasOwnProperty(a)}function f(e){return","==e?c(a,f):g(e)?d?l():c():c(n(b))}return function(b){return g(b)?d?l():c():l(a,f)}}var r,x,f={state:null,column:null,marked:null,
cc:null},D=/^end[\w]+$/,H={live:!0,footer:!0,comment:!0},J={"<":!0,">":!0,"<=":!0,">=":!0,"==":!0,contains:!0,and:!0,or:!0};return{startState:function(){return{tokenize:t,cc:[],liquidMode:null,ac:null}},resetState:function(a){a.tokenize=t;a.cc=[];a.liquidMode=null;a.ac=null;return a},token:function(a,b){r=null;a.sol()&&this.resetState(b);var c=b.tokenize(a,b);null!=b.liquidMode&&(c=(c||"")+" liquid liquid-"+b.liquidMode);if("whitespace"==r||null==r)return c;var d;a:{d=r;var e=x,h=b.cc;f.state=b;f.stream=
a;f.marked=null;for(f.cc=h;;)if((h.length?h.pop():B)(d,e)){for(;h.length&&h[h.length-1].ac;)h.pop()();if(f.marked){d=f.marked;break a}d=null;break a}}return(c||"")+" "+(d||"")}}});CodeMirror.defineMIME("application/x-liquid-template","liquid");CodeMirror.defineMode("htmlmixedliquid",function(d,k){return CodeMirror.overlayParser(CodeMirror.getMode(d,k.backdrop||"text/html"),CodeMirror.getMode(d,k.overlay||"application/x-liquid-template"))});
CodeMirror.modeExtensions&&(CodeMirror.modeExtensions.htmlmixedliquid=CodeMirror.modeExtensions.htmlmixed,CodeMirror.defineExtension("getModeExtAtPos",function(d){return(d=this.getTokenAt(d))&&d.state&&d.state.mode?CodeMirror.modeExtensions["html"==d.state.mode?"htmlmixed":d.state.mode]:d&&d.state&&d.state.base&&d.state.base.mode?CodeMirror.modeExtensions["html"==d.state.base.mode?"htmlmixed":d.state.base.mode]:this.getModeExt()}));
CodeMirror.defineExtension("commentRangeLiquid",function(d,k,e){this.getModeExtAtPos(this.getCursor());if(d)this.getRange(k,e),this.replaceRange("{% comment %}"+this.getRange(k,e)+"{% endcomment %}",k,e),k.line==e.line&&k.ch==e.ch&&this.setCursor(k.line,k.ch+13);else{d=this.getRange(k,e);var m=d.indexOf("{% comment %}"),q=d.lastIndexOf("{% endcomment %}");-1<m&&-1<q&&q>m&&(d=d.substr(0,m)+d.substring(m+13,q)+d.substr(q+16));this.replaceRange(d,k,e)}});
